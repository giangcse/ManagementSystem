    ...
    include('sql_conn.php'); //Kết nối tới Database
      ...
      if ($result->num_rows > 0) {
          while($row = mysqli_fetch_assoc($result)) {
            if ($row["E_ACCOUNT"] == $_POST['username']) { //Kiểm tra account
              if ($row["E_PASSWORD"] == sha1($_POST['password'])) { //Kiểm tra password
                  $_SESSION['account'] = $row['E_ACCOUNT']; //Lưu tên đăng nhập
                    if($row['E_USERGROUP'] == "admin" || $row['E_USERGROUP']=="order"){
                      $_SESSION['user_group']=$row['E_USERGROUP'];
                      header('Location: index.php');
                    }
                    elseif($row['E_USERGROUP'] == "off")
