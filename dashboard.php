                  ...
                  <div class="tab-pane active" id="order">
                    ...
                    <table class="table table-hover table-bordered">
                      ...
                      <tbody>
                        ...
                        ?>
                          <tr data-toggle="modal" data-target="#view<?php echo $row['B_ID']; ?>">
                            <th><?php echo $row['B_ID']; ?></th>
                            <td><?php echo date('h:i:s d/m/Y', strtotime($row['B_TIME'])); ?></td>
                            <td><?php echo number_format($row['B_TOTAL'], 0); ?>đ</td>
                          </tr>
                          <!-- The Modal -->
                          <div class="modal fade" id="view<?php echo $row['B_ID']; ?>" tabindex="-1" role="dialog" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-scrollable">
                              <div class="modal-content">
                                <!-- Modal Header -->
                                <div class="modal-header">
                                  <h4 class="modal-title">Chi tiết hóa đơn #<?php echo $row['B_ID']; ?></h4>
                                  <button type="button" class="close" data-dismiss="modal">&times;</button>
                                </div>
                                <!-- Modal body -->
                                <div class="modal-body">
                                    <?php
                                      $billdetail_sel = "SELECT * FROM product INNER JOIN detail_bill ON detail_bill.P_ID = product.P_ID LEFT JOIN detail_shift ON detail_shift.S_ID = detail_bill.S_ID RIGHT JOIN employees ON detail_shift.E_ID = employees.E_ID WHERE detail_bill.B_ID = '".$row['B_ID']."'";
                                      $billdetail_que = mysqli_query($conn, $billdetail_sel);
                                      if($billdetail_que->num_rows > 0)
                                        while ($rows = mysqli_fetch_assoc($billdetail_que)) {
                                          $E_FULLNAME = $rows['E_FULLNAME'];
                                    ?>
                                      <ul class="text-left">
                                        <li><h4><?php echo $rows['P_NAME']; ?></h4></li>
                                        <li>Giá: <?php echo number_format($rows['P_PRICE'], 0); ?>đ</li>
                                        <li>Số lượng: <?php echo $rows['DB_QUANTITY']; ?></li>
                                        <li>Thành tiền: <?php echo number_format($rows['DB_TOTAL'], 0); ?>đ</li>
                                      </ul>
                                      <hr>
                                    <?php
                                         } 
                                    ?>
                                    <h4 class="text-left">Nhân viên lập: <?php echo $E_FULLNAME; ?></h4>
                                    <h4 class="text-right">Tổng tiền: <?php echo number_format($row['B_TOTAL'], 0); ?>đ</h4>
                                </div>
                                <!-- Modal footer -->
