        ...
        <div class="collapse navbar-collapse" id="navbarNav">
          ...
          <ul class="navbar-nav col-auto">
            <li class="nav-item dropdown">
                       ...
                       $queryName = mysqli_query($conn, $name); //Thực hiện câu truy vấn
                        if ($queryName->num_rows > 0) { //Kiểm tra số dòng
                         while ($row = mysqli_fetch_assoc($queryName)) { //Nếu có kết quả thì đưa tất cả vào bảng
						  echo "Xin chào ".$row['E_FULLNAME']; //Đúng thì cho phép truy cập và hiện xin chào
						  $E_USERGROUP = $row['E_USERGROUP']; //Đặt nhóm người dùng
						  $E_ID = $row['E_ID']; //ID user
                        }
                       }        
                     }else{
				...
				<?php
					if ($E_USERGROUP=="admin" || $E_USERGROUP=="order") 
						echo '<a class="dropdown-item" href="index.php">Đặt hàng</a>';
				  ?>
        ...
        <div class="tab-content text-center">
          <div class="tab-pane active" id="shift">
			...
			<table class="table table-hover table-bordered">
			  ...
			  <tbody>
			  	<?php 
			  		$select_ca = "SELECT DISTINCT DS_SHIFTNAME FROM detail_shift WHERE E_ID = '".$E_ID."'";
			  		...
			  		if ($query_ca->num_rows > 0) {
			  			...
			  			<tr>
			  				...
			  				<td>
			  					<?php 
			  					 	$select_ngay = "SELECT DS_DAYOFSHIFT FROM detail_shift WHERE DS_SHIFTNAME = '".$row['DS_SHIFTNAME']."' AND E_ID = '".$E_ID."'";
